  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(loginEim.fulfilled, (state, action) => {
        ...
        state.loadding = false;
        state.isAuthentication = true;
        setLocalStorage({ key: Authenticate.AUTH, data: accessToken });
        setLocalStorage({ key: Authenticate.REFRESH_TOKEN, data: refreshToken });
        // getTransId(action.payload.data.accessToken) && setLocalStorage({
        //   key: EIM.SITE_MAP,
        //   data: getTransId(action.payload.data.accessToken),
        // });
      })
